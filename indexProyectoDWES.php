<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="webroot/css/index.css" type="text/css">
    <title>Víctor García Gordón</title>
</head>
<body>
    <header>      
        <h1 id="inicio">Desarrollo Web en Entorno Servidor</h1>
    </header>
    <main>
        <nav>
            <h2>TEMA 1 DESARROLLO WEB EN ENTORNO SERVIDOR</h2>
            <ul>
                <li><a target="blank" href="doc/EstudioTema1.pdf">Desarrollo de aplicaciones web (estudio teórico)</a></li>
                <li><a target="blank" href="doc/Prestashop.pdf">Prestashop (estudio teórico)</a></li>
            </ul>
            <h2>TEMA 2 INSTALACIÓN, CONFIGURACIÓN Y DOCUMENTACIÓN DEL ENTORNO DE DESARROLLO Y DEL ENTORNO DE EXPLOTACIÓN</h2>
            <ul>
                <li><a target="blank" href="doc/EstudioTema2.pdf">Entorno de desarrollo AMP</a></li> 
                <li><a target="blank" href="doc/EstudiosClasesyFunciones.pdf">Funciones y Clases PHP</a></li>
                <li><a target="" href="">Entorno de desarrollo Prestashop</a></li>
                <li><a target="blank" href="https://prestashop.victorgargon.informatica.ieslossauces.es/">Tienda Prestashop</a></li>
                <li><a target="blank" href="https://biblioteca.victorgargon.informatica.ieslossauces.es/">Sistema Biblioteca</a></li>
            </ul> 
            <h2>TEMA 3 CARACTERÍSTICAS DEL LENGUAJE PHP</h2>
            <ul>
                <li><a href="/202DWESProyectoTema3/indexProyectoTema3.php">Ejercicios Tema 3</a></li>        
            </ul>  
            <h2>TEMA 4 TÉCNICAS DE ACCESO A DATOS EN PHP</h2>
            <ul>
                <li><a href="/202DWESProyectoTema4/indexProyectoTema4.php">Ejercicios Tema 4</a></li>        
            </ul>
            <h2>TEMA 5 DESARROLLO DE APLICACIONES WEB UTILIZANDO CÓDIGO EMBEBIDO</h2>
            <ul>
                <li><a href="/202DWESProyectoTema5/indexProyectoTema5.php">Ejercicios Tema 5</a></li>
                <li><a href="/202DWESLoginLogoffTema5/indexProyectoLoginLogoffTema5.php">Aplicación LoginLogoff</a></li>
            </ul> 
            <h2>TEMA 6 PROGRAMACIÓN ORIENTADA A OBJETOS EN PHP</h2>
            <ul>
                <li><a href="/202DWESLoginLogoff/indexLoginLogoff.php">Aplicación LoginLogoff MVC</a></li>     
            </ul> 
            <h2>TEMA 7 PROGRAMACIÓN DE SERVICIOS WEB </h2>
            <ul>
                <li><a href=""></a></li>        
            </ul> 
            <h2>TEMA 8 DESARROLLO DE APLICACIONES WEB HÍBRIDAS</h2>
            <ul>
                <li><a href=""></a></li>        
            </ul> 
            <h2>TEMA 9 DESARROLLO DE APLICACIONES WEB HÍBRIDAS</h2>
            <ul>
                <li><a href=""></a></li>        
            </ul> 
        </nav>       
    </main>
    <footer>
        <div>
            <a href="/index.html">Víctor García Gordón</a>
            <a target="blank" href="doc/curriculum.pdf"><img src="doc/curriculum.jpg" alt="curriculum"></a>
            <a target="blank" href="https://github.com/victorgargor/202DWESProyectoDWES"><img src="doc/github.png" alt="github"></a>
            <a target="blank" href="https://github.com">Web Imitada</a>
        </div>
    </footer>
</body>
</html>
